...
class RoomService {
  ...
  async randomOccupancy() {
    try {
      // Additive: Only fetch rooms that are currently available
      // Do NOT reset existing bookings or occupied rooms
      const availableRooms = await Room.findAvailable();

      if (availableRooms.length === 0) {
        return { occupiedCount: 0, occupiedRooms: [] };
      }

      // Fisher-Yates shuffle on available rooms
      ...
      // Mark a random subset of AVAILABLE rooms as occupied (e.g., 20% of available)
      const countToOccupy = Math.max(1, Math.floor(shuffled.length * 0.2));
      const roomsToOccupy = shuffled.slice(0, countToOccupy);
      const roomNumbers = roomsToOccupy.map((r) => r.roomNumber);

      // Update database
      await Room.bulkUpdateStatus(roomNumbers, 'occupied');

      logger.info(`Added random occupancy to ${countToOccupy} rooms`);
      return {
        occupiedCount: countToOccupy,
        occupiedRooms: roomNumbers,
      };
    } catch (error) {
      logger.error(`Error adding random occupancy: ${error.message}`);
      throw error;
    }
  }
